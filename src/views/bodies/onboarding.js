...
export class pantallaOnboarding extends connect(store, SCREEN, MEDIA_CHANGE, PUBLICACIONES_TIMESTAMP)(LitElement) {
    ...
    static get styles() {
        return css`
            ...
            :host(:not([media-size="small"])) #header {
            ...
            :host([media-size="medium"]) #btn-cuenta {
                width: 12rem;
            }
            :host([media-size="large"]) #btn-siguiente,
            :host([media-size="large"]) #btn-cuenta {
                width: 12rem;
            }
            #btn-cuenta {
                position: relative;
    ...
    render() {
        if (this.lineas.length > 0) {
            return html`
                <div id="header" style="background-image:url(${this.lineas[this.lineaActual].Imagen})"></div>
                <div id="cuerpo">
                    <div id="flecha-izq" @click="${this.atras}"></div>
                    <div id="flecha-der" @click="${this.adelante}"></div>
                    <div id="titulo">${this.lineas[this.lineaActual].Titulo}</div>
                    <div id="leyenda">${this.lineas[this.lineaActual].Leyenda}</div>
                    <div id="puntos">
                        ${repeat(
                            this.lineas,
                            (item) => item.Titulo,
                            (item, index) => html` <div id="punto${index}" class="${index == 0 ? "puntoLLeno" : "puntoVacio"}"></div> `
                        )}
                    </div>
                    <button id="btn-siguiente" btn3 @click=${this.clickBoton1}>${idiomas[this.idioma].onboarding.btn1}</button>
                    <button id="btn-cuenta" btn1 @click=${this.clickBoton2}>${idiomas[this.idioma].onboarding.btn2}</button>
                </div>
            `;
        }
    }
    ...
    adelante() {
        if (this.lineaActual + 1 < this.lineas.length) {
            ...
            this.shadowRoot.querySelector("#header").style.backgroundImage = "url(" + this.lineas[this.lineaActual].Imagen + ")";
            if (this.lineaActual + 1 == this.lineas.length) {
                this.shadowRoot.querySelector("#flecha-der").hidden = true;
