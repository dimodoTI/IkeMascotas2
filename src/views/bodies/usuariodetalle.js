...
import { setDatos } from "../../redux/cliente/actions";
...
import { getCantidad } from "../../redux/mascotas/actions";

import { FOTOEDIT } from "../../../assets/icons/icons";
...
export class pantallaUsuariodetalle extends connect(store, SCREEN, MEDIA_CHANGE, CLIENTE_LOGUEADO, UPDATEPROFILE_TIMESTAMP, FOTOS_TIMESTAMP, GETCANTIDAD_TIMESTAMP)(LitElement) {
    ...
    render() {
        return html`
            <div id="cuerpo">
                ...
                <div class="ikeInput">
                    <label id="lblDocumento">${idiomas[this.idioma].usuariodetalle.lblDocumento}</label>
                    <input id="txtDocumento" @input=${this.activar} type="number" placeholder=${idiomas[this.idioma].usuariodetalle.lblDocumento_ph} .value="${this.item.documento}" />
                    <label id="lblErrorDocumento" error oculto>Documento Incorrecto</label>
                </div>

                <button id="btnClave" btn2 style="justify-self:end" @click=${this.clickClave}>${idiomas[this.idioma].usuariodetalle.btnClave}</button>
                <button id="btnEdit" btn1 @click=${this.clickEdit}>${idiomas[this.idioma].usuariodetalle.btnEdit}</button>
                <button id="btnCobertura" btn3 @click=${this.clickCobertura}>${idiomas[this.idioma].usuariodetalle.btnCobertura}</button>
                <button id="btnDelete" btn2 @click=${this.clickDelete}>${idiomas[this.idioma].usuariodetalle.btnDelete}</button>

                <div id="divAyuda">${idiomas[this.idioma].usuariodetalle.lblAyuda}</div>

                <button id="btnAsistencia" btn3 @click=${this.clickAsistencia}>${idiomas[this.idioma].usuariodetalle.btnAsistencia}</button>
    ...
    valido() {
        ...
        let docu = this.shadowRoot.getElementById("txtDocumento");
    ...
    activar() {
        ...
        let docu = this.shadowRoot.getElementById("txtDocumento");
        if (nom.value != "" && mail.value != "" && cel.value != "" && docu.value != "") {
    ...
    asignarValores(item) {
        ...
        item.documento = this.shadowRoot.querySelector("#txtDocumento").value;
