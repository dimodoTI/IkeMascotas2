...
import { upload, delCliente, borrarAdjunto as patchAdjuntos } from "../../redux/adjuntos/actions";

import { button } from "../css/button";
import { cardArchivo } from "../css/cardArchivo";

import { ATRAS, ARCHIVO, TRASH } from "../../../assets/icons/icons";
    ...
    render() {
        return html`
            ...
            <div id="contenedor" style="width:100%;padding:5rem;display:grid;grid-template-rows: 5% 5% 80%;grid-gap: .5rem;">
                <div id="titulo">${idiomas[this.idioma].consultaMsg.titulo}</div>
                <div id="horario" style="display:grid;justify-items:center">
                    <label style="margin-top:.5rem"> ${idiomas[this.idioma].consultaMsg.subTitulo + this.formateoFecha(this.items.FechaAtencion) + " " + idiomas[this.idioma].consultaMsg.hora + this.formateoHora(this.items.HoraAtencion)}</label>
                </div>
                <div id="divDetalle">
                    <div id="listaAdjuntos" style="padding-top:.5rem;display:grid;grid-gap:.5rem">
                        ${this.adjuntosCliente.map(
                            (dato) => html`
                        ...
                        <form id="form" name="form" action="/uploader" enctype="multipart/form-data" method="POST" style="justify-self: center;">
                            ...
                            <button type="button" id="btn-adjuntar" btn3 @click=${this.adjuntar}>${idiomas[this.idioma].consulta.btn1}</button>
                        </form>
                    </div>
                </div>
            </div>
