...
import { edit as mascotasEdit, getCombo } from "../../redux/mascotas/actions";

import { enAtencion as getEnAtencion, agendarReserva } from "../../redux/reservas/actions";
import { goNext, goTo } from "../../redux/routing/actions";
import { isInLayout } from "../../redux/screens/screenLayouts";
import { showScreen } from "../../redux/screens/actions";

import { pantallaListaReserva } from "./listaReserva";
import { limpiarFoto } from "../../redux/fotos/actions";
...
export class pantallaMascotaVer extends connect(store, SCREEN, MEDIA_CHANGE, VACUNAS_MASCOTAS_TIMESTAMP, CALENDARIO_TIMESTAMP, COMBO_MASCOTAS)(LitElement) {
	...
	render() {
		return html` <div id="cuerpo" media-size="${this.mediaSize}">
			<div style="display:grid;grid-auto-flow:row;align-content: start;">
				<div id="foto" style="background-image:url(${this.items.Foto})"></div>
				...
				<button id="btn-edit" btn1 @click=${this.clickEdit}>${idiomas[this.idioma].mascotaver.btn1}</button>
			</div>
			...
			<div style="display:grid;grid-auto-flow:row;grid-gap:.3rem">
				...
				<div style="padding:.5rem 0 .5rem 0">
					...
					<div style="display:grid;grid-gap:.8rem;">${this.vacunas()}</div>
					<button id="btn-edit" btn3 @click=${this.clickVacunas}>${idiomas[this.idioma].mascotaver.btn3}</button>
				</div>
			</div>
		</div>`;
	}
	...
	vacunas() {
		return this.mascotasVacuna.map((dato) => {
			...
			return html` <div id="ccDivEtiqueta" style="width:90%;justify-self:center">
				<div id="ccDivVacuna">${dato.Vacuna ? dato.Vacuna.Descripcion : ""}</div>
				<div id="ccDivPara">${calendario.length > 0 ? calendario[0].Enfermedades : ""}</div>
				<div class="labelRedondeado" id="ccDivCachorro">${calendario.length > 0 ? calendario[0].Edad : ""}</div>
	...
	clickConsulta() {
		if (store.getState().cliente.datos.cobertura.length > 0 && store.getState().cliente.datos.cobertura[0].Cobertura) {
			store.dispatch(
				getCombo({
					orderby: "Nombre",
					select: "Id,Nombre",
			...
			store.dispatch(showWarning("cobertura", 0, "fondoError", 0));
		}
	}
	clickVacunas() {
		store.dispatch(
