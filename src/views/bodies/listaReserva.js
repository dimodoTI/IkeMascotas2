...
import { CHAT, VIDEO, ARCHIVO, TRASH } from "../../../assets/icons/icons";
...
import { chatReserva, CHAT_RESERVAM_SUCCESS, CHAT_RESERVAR_SUCCESS } from "../../redux/chat/actions";
                   ...
                   <div id="cmhDivEtiqueta" .item="${dato}" @click="${this.mostrarIcono}">
                        ...
                        <div id="cmhDivNombre">${dato.Mascota.Nombre}</div>
                            ...
                            <div id="cmhDivVerDetalle">
                                <button btn2  @click=${this.verDetalle} .item=${dato} style="width:4rem;padding:0;text-align:left;font-size: var(--font-label-size);font-weight: var(--font-label-weight);cursor:pointer">${idiomas[this.idioma].listaReserva.verDetalle}</button>                    
                            </div>
                            <div id="cmhDivChat" style="cursor:pointer" @click=${this.atencion} .item=${dato} ?hiddenchat="${this.escondoVideo(dato.FechaAtencion, dato.Atencion)}">${VIDEO}</div>                    
                            <div id="cmhDivChat" style="cursor:pointer"  @click=${this.chat} .item=${dato}  ?hiddenchat="${!this.muestroChat(dato)}">${CHAT}</div>   
